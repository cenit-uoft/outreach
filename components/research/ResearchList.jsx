...
export default () => {
  ...
  useEffect(() => {
    const thrusts = getPagesUnderRoute("/research");
    thrusts.sort((a, b) => {
      return (
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-8">
        <div className="max-w-3xl mx-auto sm:text-center">
          <h3 className="text-gray-800 text-3xl font-semibold sm:text-4xl">
            CENIT Research Thrusts
          </h3>
          <p className="text-gray-600 mt-3">CENIT is organized into four research thrusts to study the use of computing power and machine intelligence to develop digital information infrastructures that are more efficient, secure, resilient, and affordable and available to all segments of society. </p>
        </div>
        <div className="mt-12">
          <ul className="grid gap-12 sm:grid-cols-2 md:grid-cols-2">
            {Array.isArray(thrusts) && thrusts.map((page, idx) => (
              <li key={idx} className="transition transform hover:-translate-y-1 duration-200">
                ...
                <a href={page.route} className="block group hover:no-underline">
                  ...
                  <div className="w-full h-80 sm:h-52 md:h-56 overflow-hidden rounded-xl shadow-md group-hover:shadow-lg transition duration-200">
                    <img
                      src={page.frontMatter?.image}
                      className="w-full h-full object-cover object-center group-hover:scale-110 transition duration-100"
                      alt={page.frontMatter?.title || "Research Image"}
                    />
                  </div>
                  
                  <div className="mt-4">
                    <h4 className="text-xl text-gray-700 font-semibold group-hover:text-[#35538E] transition duration-100">
